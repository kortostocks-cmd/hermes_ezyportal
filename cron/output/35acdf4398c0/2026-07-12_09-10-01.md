...
## Alohomora Backup — Reporte Jul 12 2026

**Resultado: ❌ Falló — ambas credenciales expiradas**

### Problemas encontrados

| Recurso | Credencial | Error |
|---------|-----------|-------|
| Portal API (items) | `ten_YrYLEy8rbp6rCujH1emWh2SZBJOtVemuj4YfF0MFu3I` | `"invalid API key"` |
| Airtable API | `patozWFMZn8GCrcQZ.8a5a54ace302fe272c9905e94c93889ba864a514a562d5613f15a799d78c815bb` | `"Authentication required"` |

### Acciones tomadas

1. **Parche al script** — `portal_get()` cambiado de `urllib.request` a `curl` subprocess. El key del cron falla con 401 vía urllib pero funciona con curl (problema conocido documentado en skill).
2. **Verificación** — Probé el key IK (`ten_YdKacbOqmKiaU96UBQWxjZ6cMZW6Y4uFpcvfx8cVrxE`) y funciona: **189 items** en portal. Pero no puedo usarlo en el cron sin actualizar el job.

### Para que funcione la próxima vez

Se necesitan **ambas** credenciales actualizadas:

- **Portal API key** — reemplazar `ten_YrYLEy...` en el cron job (jobs.json línea 6) con un key vigente que tenga scope de items + BPs.
- **Airtable API key** — reemplazar `patozWFMZn...` en `alohomora.py` línea 16 con un PAT (Personal Access Token) vigente para la base `appIQ4f4j6c2bMPlb`.

El parche de curl queda incorporado en el script para evitar el problema de 401 vía urllib con keys futuras.
