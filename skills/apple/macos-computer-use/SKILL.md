...
## Discord Integration (background automation via gateway)

When the Discord bot is the only interface (user is remote) and `computer_use` is
unavailable (macOS 12.x or earlier, CuaDriver.app not installable), you can still
give the Discord bot terminal/file/web access. Configure the platform toolsets:

```bash
hermes config set platform_toolsets.discord "['hermes-discord', 'terminal', 'file', 'web']"
hermes gateway restart
```

This lets the Discord bot run shell commands, read/write files, and browse the web
on the Mac — but NOT drive the GUI visually (no `computer_use`). The Discord bot
connects as `HERMES#XXXX` and responds to @mentions.

**Token placement matters**: For the profile-specific bot, the Discord token goes in
`~/.hermes/profiles/<profile>/.env` (NOT `~/.hermes/.env`). The profile `.env` takes
precedence. Verify with:
```bash
grep DISCORD_BOT_TOKEN ~/.hermes/profiles/<profile>/.env
```
Restart the gateway after changing the token: `hermes gateway restart`

**When Discord shows "Improper token"**: Check BOTH `.env` files. If both have
`DISCORD_BOT_TOKEN`, the profile one wins. A truncated or wrong token in either
causes 401. Use `terminal()` (not `execute_code`) to write tokens to files to avoid
Python string interpolation silently truncating long strings.
...
- **"cua-driver: The application cannot be opened for an unexpected reason,
  error=... kLSIncompatibleSystemVersionErr"** — CuaDriver.app (the GUI
  component with TCC grants) requires **macOS 13 Ventura or later**. Your
  Mac has an older version. Workaround: all `computer_use` actions that
  don't need the GUI app will still work with the CLI binary on older
  macOS, but the full background-automation capability (no cursor steal,
  no focus change) requires Ventura+.
  - Check your version: `sw_vers`
  - If on macOS 12.x (Monterey) or earlier: use Discord with
    `terminal`/`file`/`web` toolsets only. Configure via:
    `hermes config set platform_toolsets.discord "['hermes-discord', 'terminal', 'file', 'web']"`
  - The CLI binary (`~/.local/bin/cua-driver`) still works on older macOS for
    local CLI sessions — only the app's TCC identity (for permission prompts)
    requires Ventura+.
