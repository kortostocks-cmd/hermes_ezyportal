...
- **Discord bot stuck retrying / failed to reconnect**: Check `gateway_state.json` for `platforms.discord.state`. If it shows `retrying` with `failed to reconnect`, update the bot token in `.env` (use Python/terminal since `read_file` is blocked for credentials), then restart the profile gateway: `hermes gateway restart --profile <name>`. Verify state switches to `connected` with no error_message.
